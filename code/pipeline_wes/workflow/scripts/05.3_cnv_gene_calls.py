...
def main(args):
    ...
    # for unknown reasons, FACETS sometimes returns segments with end = start -1
    # this causes in error when executing bedtools commands
    mask_bad_segs  = df_bed["start"]>=df_bed["end"]
    if sum(mask_bad_segs)>0:
        print("WARNING! the following segments have negative size!")
        print(df_bed.loc[mask_bad_segs, ["chrom", "start", "end", "tcn.em", "lcn.em", "svlen"]])
        df_bed = df_bed.loc[~mask_bad_segs]
