...
rule pon_mutect2:
    ...
    conda:
        "../envs/main.yaml"
    params:
        java="'-Xmx30g -Djava.io.tmpdir=%s'" % config["general"]["tmpdir"]
    threads: 4
    resources:
        queue="shortq",
        mem_mb=20000,
...
rule pon_genomicsdb:
    ...
    conda:
        "../envs/main.yaml"
    params:
        java="'-Xmx25g -Djava.io.tmpdir=%s -DGATK_STACKTRACE_ON_USER_EXCEPTION=true'" % config["general"]["tmpdir"],
    ...
    shell:
        ...
        gatk --java-options {params.java} GenomicsDBImport \
            ...
            --overwrite-existing-genomicsdb-workspace &> {log}
        """
...
rule pon_create:
    ...
    conda:
        "../envs/main.yaml"
    params:
        java="'-Xmx50g -Djava.io.tmpdir=%s'" % config["general"]["tmpdir"],
