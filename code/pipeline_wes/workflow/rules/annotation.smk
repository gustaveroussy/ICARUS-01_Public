...
# Add tumor pileup to maf
rule somatic_maf_add_pileup:
    input:
        maf="%s/annotation/somatic_maf/{tsample_1}_vs_{nsample}.maf" % R_FOLDER,
        pup="%s/calling/somatic_run_mpileup_tumor/{tsample_2}_on_{tsample_1}_vs_{nsample}_positions.mpileup.tsv.gz" % R_FOLDER,
    output:
        maf="%s/annotation/somatic_maf_add_pileup/{tsample_1}_vs_{nsample}_w_pileup_{tsample_2}.maf" % R_FOLDER
    benchmark:
        "%s/annotation/somatic_maf_add_pileup/{tsample_1}_vs_{nsample}_w_pileup_{tsample_2}.tsv" % B_FOLDER
    log:
        "%s/annotation/somatic_maf_add_pileup/{tsample_1}_vs_{nsample}_w_pileup_{tsample_2}.log" % L_FOLDER
    conda:
        "../envs/python.yaml"
    threads: 1
    params:
    ...
    shell:
        """
        python -u workflow/scripts/03.4_maf_add_pileup.py \
            --inp_maf {input.maf} \
            --inp_pup {input.pup} \
            --out_maf {output.maf} &> {log}
        """
